...
def extract(url, table_attribs):
    ...
    for row in rows:
        col = row.find_all('td')
        if len(col) != 0:
            name = col[1].text.strip() if col[1] else None
            data_dict = {"Name":name,
                        "MC_USD_Billion":float(col[2].contents[0].rstrip('\n'))}
...
def transform(df, csv_path):
    ...
    exchange_rate_dict = exchange_rate_df.set_index(exchange_rate_df.columns[0]).to_dict()[exchange_rate_df.columns[1]]
